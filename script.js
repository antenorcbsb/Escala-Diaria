...
function updateViewer(loaded) {
    ...
    if (badgeText) {
        badgeText.innerText =
            `Turno Atual: ${formattedDate} (${shift})`;
    }
...
async function loadPdfList() {
    ...
    try {
        ...
        if (!data.success) {
            throw new Error(
                'O Google Apps Script devolveu um erro.'
            );
        }
...
function scheduleNextShift(loaded) {
    ...
    const nextHour =
        String(nextChange.getHours()).padStart(2, '0');

    const nextMinute =
        String(nextChange.getMinutes()).padStart(2, '0');

    console.log(
        `Próxima mudança: ${nextHour}:${nextMinute} (daqui a ${hours}h ${minutes}min)`
    );
    ...
    setTimeout(async () => {
        ...
        const updated = await loadPdfList();

        if (updated) {

            updateViewer(updated);

            scheduleNextShift(updated);

        } else {

            setStatus(
                'error',
                'Erro ao atualizar a escala'
            );
